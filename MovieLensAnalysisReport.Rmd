...
Regularized movie and user effect model have be used for final model. We will be using full cross-validation just on the **train_set**, without using the **test_set** until the final assessment as the test set should never be used for tuning.

**test_set** set will be used for RMSE calculation for *lambda* parameter tuning. From there, minimum value of RMSE achieving *lambda* will be used for final model training on **edx** data set and Final RMSE **validation** set will be used.
...
### 3.4. Results of training of **edx** set and **validation** set

### 3.4.1. 

Now, we are using the *lambda* value gained using cross validation. We are preparing the values for linear model and fitting the model. Final RMSE is 
